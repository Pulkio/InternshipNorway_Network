...
public:
    ...
    int ListenForMessages(UMyGameInstance* gameInstanceArg);

    // Reset les compteurs de réponses
    void ResetCounters() { countReponse1 = 0; countReponse2 = 0; }

private:
    // Un objet SOCKET qui représente la connexion au serveur.
    SOCKET sock;
    TQueue<FString> messageQueue;
    FCriticalSection messageQueueMutex;
