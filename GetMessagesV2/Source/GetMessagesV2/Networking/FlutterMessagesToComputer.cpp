...
int FlutterMessagesToComputer::SetupServerAddress() {
    ...
    inet_pton(AF_INET, "adresse IP server", &(serverAddress.sin_addr)); // Adresse IP du serveur
    return 0;
}
...
//Ecoute les messages et mets à jour le HUD
int FlutterMessagesToComputer::ListenForMessages(UMyGameInstance* gameInstance) {
        ...
        if (iResult > 0) {
            ...
            if (GEngine != nullptr)
            {
                if (gameInstance != nullptr)
                {
                    ...
                    if (Message == "Reponse 1") {
                        countReponse1++;
                    }
                    else if (Message == "Reponse 2") {
                        countReponse2++;
                    }
                    FString newMessage = "Nb Rep1: " + FString::FromInt(countReponse1) +
                        " | Nb Rep2: " + FString::FromInt(countReponse2);
                    gameInstance->SetLastMessage(FText::FromString(newMessage));
                }
                else {
                    UE_LOG(LogTemp, Warning, TEXT("GameInstance est null 2 "));
                }
            }
            else {
